...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (h.length !== 3 && h.length !== 6) return `rgba(132, 204, 22,${alpha})`;
...
export default function PageViewMap({ data, height = 380 }: PageViewMapProps) {
  ...
  useEffect(() => {
    ...
    const accent = (cs?.getPropertyValue('--admin-rail').trim() || '#84cc16');
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '3px solid var(--admin-rail, #84cc16)',
          borderRadius: '2px',
